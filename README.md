...
`vercel.json` настраивает задания Vercel Cron (роуты защищены `CRON_SECRET`,
Vercel подставляет его в `Authorization: Bearer ...`):

- `/api/cron/bot` — ежедневно 07:00 UTC: напоминания и групповой ростер по
  настройкам «Бот» (модель `BotSettings`, админка). Срабатывает в выбранный
  **день недели** (таймзона команды), дедуп по локальной дате. На плане Hobby
  крон идёт раз в сутки, поэтому час — ориентировочный (= время запуска крона);
  на Pro можно поставить `0 * * * *` и соблюдать час точно.
...
> для совместимости, но по умолчанию не запланирован; актуальный механизм —
> `/api/cron/bot`, настраиваемый из админки.
